"""Shared chat-LLM base.

Both backends (Anthropic Claude, Azure AI Foundry) speak the same
system+user → JSON/text protocol and share identical prompts, JSON extraction,
and classification parsing. The only thing that differs is the low-level API
call, so backends subclass :class:`ChatJSONClient` and implement just
``_text_call``. The prompts here are what enforce the "only these two
categories" precision — edit them carefully and re-check against ``tests/``.
"""
...
class ChatJSONClient(LLMClient):
    """Provider-agnostic implementation of the :class:`LLMClient` protocol.

    Subclasses provide ``_text_call(system, user) -> Optional[str]`` (the actual
